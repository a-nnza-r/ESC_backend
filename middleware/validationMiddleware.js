export function validateJSON_createEPF(req, res, next) {
    ...
    try {
        ...
        const isPayloadValid = expectedKeys.every(key => incomingKeys_createEPF.includes(key))
        if(!isPayloadValid) {
            return res.status(400).json({error: "Invalid JSON payload format for createEPF"})
        } else {
            next()
        }
    } catch (e) {
...
export function validateJSON_updateEPF(req, res, next) {
    ...
    try {
        ...
        const isPayloadValid = expectedKeys.every(key => incomingKeys_updateEPF.includes(key))
        if(!isPayloadValid) {
            return res.status(400).json({error: "Invalid JSON payload format for updateEPF"})
        } else {
            next()
        }
    } catch (e) {
...
export function validateParam_getEPF(req, res, next) {
    try {
        if(!req.query.hasOwnProperty('epf_id')) {
            return res.status(400).json({error: "Missing epf_id key for get EPF query"})
        } else {
            next()
        }
    } catch (e) {
...
export function validateParam_deleteEPF(req, res, next) {
    try {
        if(!req.query.hasOwnProperty('epf_id')) {
            return res.status(400).json({error: "Missing epf_id key for delete EPF query"})
        } else {
            next()
        }
    } catch (e) {
