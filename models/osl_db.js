...
export async function createOSL(name, email, pool = defaultPool) {
  ...
  try {
    ...
    const duplicateCheckQuery =
      "SELECT * FROM OSL WHERE name = $1 AND email = $2";
    ...
    const query = "INSERT INTO OSL (name, email) VALUES ($1, $2) RETURNING *";
...
export async function getOSLs(pool = defaultPool) {
  const client = await pool.connect();
  try {
    const result = await client.query(
      "SELECT * FROM OSL WHERE is_deleted = false;"
    );
    return result.rows;
  } finally {
    client.release();
...
export async function getOSL(user_id, pool = defaultPool) {
  if (!user_id) {
...
export async function updateOSL(user_id, name, email, pool = defaultPool) {
  ...
  try {
    ...
    if (response.rowCount == 0) {
      throw new Error("No OSL record found with the given user_id.");
    }
...
export async function deleteOSL(user_id, pool = defaultPool) {
  if (!user_id) {
    throw new Error("User ID must be provided");
  }
  ...
  try {
    const query =
      "UPDATE OSL SET is_deleted = true WHERE user_id = $1 AND is_deleted = false RETURNING *;";
    const res = await client.query(query, [user_id]);

    if (res.rowCount === 0) {
      const query =
        "SELECT * FROM OSL WHERE user_id = $1 AND is_deleted = true ;";
      const resCheck = await client.query(query, [user_id]);

      if (resCheck.rowCount > 0) {
        throw new Error("OSL user has already been delete");
      } else {
        throw new Error("OSL user does not exist");
      }
    }
