...
describe('Robustness/Fuzzy testing of deleteUser API', () => {
  ...
  test('Test ID 1: Call next() when "user_id" key exists in request query', () => {
    ...
    validateParam_deleteUser(req, res, next);
  ...
  test('Test ID 2: Return 400 error when "user_id" key is missing from request query', () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ error: 'Missing user_id key for delete User query' });
  });
